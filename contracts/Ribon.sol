...
contract Ribon {
    ...
    function addNonProfitToWhitelist(
        address _nonProfit
    ) public {
        require(
            msg.sender == nonProfitCouncil,
            "You are not non profit council."
        );
    ...
    function removeNonProfitFromWhitelist(
        address _nonProfit
    ) public {
        require(
            msg.sender == nonProfitCouncil,
    ...
    function addDonationPoolBalance(
        uint256 _amount
    ) public payable {
    ...
    function getIntegrationCouncil()
        public
        view
        returns (address)
    {
